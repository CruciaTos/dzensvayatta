...
function FounderCard({
  ...
  function onMouseLeave() { mx.set(0); my.set(0); setHovered(false); }

  return (
    <motion.div
      ref={cardRef}
      className="relative overflow-hidden flex flex-col justify-between px-16 py-14 max-md:px-6 max-md:py-12 border-b border-border"
    ...
    >
      ...
      {/* Scan line — sweeps left→right on hover */}
      <AnimatePresence>
        {hovered && (
          <motion.div
            key="scan"
...
export function AboutOverlay({ open, onClose }: AboutOverlayProps) {
  ...
  const fadeUp = {
    ...
    show: { opacity: 1, y: 0, transition: { duration: 0.7, ease: [0.22, 1, 0.36, 1] as number[] } },
  };
          ...
          >
            ...
            >
              ...
              <motion.button
                ...
                whileHover={{ color: "#f5f5f4", borderColor: "rgba(216,211,203,0.35)" }}
                whileTap={{ scale: 0.96 }}
            ...
            <motion.div className="flex flex-col" variants={stagger} initial="hidden" animate="show">
              ...
              >
                ...
                <h2 className="font-serif text-[clamp(32px,4vw,60px)] font-normal leading-[1.0] tracking-[-0.025em] text-stone-100 mb-10">
                  {(["Intelligence built", "into the <fabric>", "of operations."] as const).map((line, i) => (
                    <div key={i} className="overflow-hidden">
                      ...
                      >
                        {i === 1
                          ? <>into the <em className="not-italic text-stone-400">fabric</em></>
                          : line}
                      </motion.div>
                    </div>
                  ))}
                ...
                >
                  DZen was founded on a single conviction: the gap between your systems is where operational value goes to die. We build the integration layer that closes that gap — connecting the tools you already run, automating the decisions you already make, and making the intelligence latent in your data finally visible and useful.
                </motion.p>
                <motion.p
                ...
                >
                  We work exclusively with mid-market and enterprise operators. No demos. No pilots. Production-grade from day one.
                </motion.p>
                  ...
                  <div className="grid grid-cols-2 max-[500px]:grid-cols-1 border border-border">
                    {COMPANY_META.map((meta, i) => (
                      <motion.div
                        ...
                        className={[
                          "px-6 py-5",
                          i % 2 === 0 ? "border-r border-border max-[500px]:border-r-0" : "",
                          i >= 2      ? "border-t border-border" : "",
                          "max-[500px]:border-b",
                          i === COMPANY_META.length - 1 ? "max-[500px]:border-b-0" : "",
                        ].join(" ")}
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
